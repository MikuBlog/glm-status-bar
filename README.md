...
![状态栏徽章样式](assets/badge.png)

- **颜色随用量自动变化**：`<50%` 青绿渐变 · `50–80%` 橙色渐变 · `>80%` 红紫渐变（告急），一眼判断额度健康度
- **左键**点击打开/收起面板，**右键**弹出快捷菜单（立即刷新 / 打开网页版 / 退出登录 / 退出应用）
- 未登录时显示半透明描边胶囊，网络异常或凭证过期时显示红色「⚠ 重试」

### 右键菜单

![右键菜单](assets/menu.png)

| 菜单项 | 说明 |
|---|---|
| **立即刷新** `⌘R` | 跳过轮询间隔，立刻拉取最新额度 |
| **打开网页版** | 浏览器打开 bigmodel.cn 用量详情页 |
| **打开 Codex / ZCode** | 启动对应编码工具；未安装时自动跳转官网下载页 |
| **退出登录 / 切换账号** | 清除本机凭证并回到登录页，可换号重登 |
| **退出 GLM StatusBar** `⌘Q` | 退出应用 |
